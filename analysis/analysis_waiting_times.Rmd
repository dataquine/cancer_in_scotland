...
waiting_times <- waiting_times_clean %>%
  ...
  mutate(
    ...
    #   quarter_new = as_factor(quarter(quarter_time)),
...
We have  **`r wt_info$count_quarters`**  quarters of Waiting Times data 
available ranging from **`r wt_info$oldest_quarter`** to **`r wt_info$newest_quarter`**.
...
# print(all_standards_totals)
# head(all_standards_totals)
```

```{r}
# all_standards_totals_combined <- all_standards_totals %>%
...
Of concern is the between 2021 and 2022 the gap between those referred and 
...
were not able to be referred. It's possible that people are presenting with 
...
treatment underway to undo the immediate delays from COVID. If there are any 
administrative processes that could be hastened or improvements in information/
communication that would be helpful for reducing the wait for patients.
...
plot_waitiing_times <- plot_waitiing_times_years_percent_treated(
...
not been achieved even as far back as 2012. 
...
needs a major rethink about how services are allocated and delivered. The 
target must have been considered achievable when it was drawn up. What has 
changed?

The 31 day target was achieved in 2022 but barely scraped pass on 95.04%.  
**The 31 day target is also at high risk of being missed in 2023** and being 
October 2023 at time of writing, little time to put new measures in place.

**Suggestion**

Set up a focus group to review what's been tried already and reasons 
for failure. It is noted that the NHS in England too is experiencing similar 
missed targets. 
There they are making adjustments to the reporting process"...following a consultation on the cancer waiting times standards..." [
